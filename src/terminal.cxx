...
char32_t read_unicode_character(void)
{
  ...
  while (true)
  {
    ...
    if (nread <= 0)
      return 0;
    if (c <= 0x7F || locale::is8BitEncoding)
    {  // short circuit ASCII
      utf8Count = 0;
      return c;
    }
...
#  endif
    ...
    else if (key == Replxx::KEY::ESCAPE)
    {  // ESC, set flag for later
      escSeen = true;
      continue;
    }
...
#  ifdef __REPLXX_DEBUG__
  if (c == ctrlChar('^'))
  {  // ctrl-^, special debug mode, prints all keys hit,
     // ctrl-C to get out
    printf(
    ...
    while (true)
    {
      ...
      for (int i = 0; i < ret; ++i)
      {
        ...
        char const* prefixText = (key < 0x80) ? "" : "0x80+";
  ...
  while (true)
  {
    ...
    switch (event)
    {
      case (WAIT_OBJECT_0 + 0):
      {
        ...
        if (
            (rec.EventType != KEY_EVENT) || (!rec.Event.KeyEvent.bKeyDown && (rec.Event.KeyEvent.wVirtualKeyCode != VK_MENU)))
        {
