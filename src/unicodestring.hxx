...
public:
  ...
  char32_t const& operator[](int pos) const
  {
  ...
  {
    return (
        (std::distance(first_, last_) <= length()) && (std::equal(first_, last_, _data.begin())));
  }
  ...
  {
    return (
        (std::distance(first_, last_) <= length()) && (std::equal(first_, last_, _data.begin(), std::forward<BinaryPredicate>(pred))));
  }
  ...
  bool ends_with(data_buffer_t::const_iterator first_, data_buffer_t::const_iterator last_) const
  {
    ...
    return (
        (len <= length()) && (std::equal(first_, last_, _data.end() - len)));
  }
